                            ...
                            struct _golsat_tree *tree)
{
    ...
    if (lbranch->visited && rbranch->visited) return NULL;
    ...
    // If the difference between the number of alive cells is greater than 10%
    //  of the average of the two branches, choose the branch with the least
    //  number of alive cells
    if (abs(left_branch_alive_cells - right_branch_alive_cells)
        > (0.1 * (left_branch_alive_cells + right_branch_alive_cells) / 2))
    {
        if (left_branch_alive_cells < right_branch_alive_cells) {
            _golsat_tree_mark_visited(rbranch);
            return lbranch;
        }
        else {
            _golsat_tree_mark_visited(lbranch);
            return rbranch;
        }
    }
